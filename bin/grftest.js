...
var GrfTest = function() {
	this.doc = window.document;
	this.tutorialsFile = "tutorials.json";
	this.tutorialsFolder = "tutorials/";
	this.div = this.doc.createElement("div");
	this.url = tink__$Url_Url_$Impl_$.parse(window.location.href);
	this.loadTutorials();
};
GrfTest.__name__ = true;
GrfTest.main = function() {
	new GrfTest();
};
GrfTest.prototype = {
	loadTutorials: function() {
		this.tutorialLoader = new nanjizal_tutorial_TutorialLoader(this.tutorialsFolder,this.tutorialsFile,$bind(this,this.tutorialsLoaded));
		this.tutorialLoader.render = $bind(this,this.renderHtml);
	}
	,tutorialsLoaded: function() {
		var names = this.tutorialLoader.itemNames();
		var id = names.indexOf(this.url.hash);
		if(id == -1) {
			id = 0;
		}
		this.tutorialLoader.loadItemByName(names[id]);
	}
...
var nanjizal_tutorial_TutorialLoader = function(tutorialsFolder_,tutorialsFile_,loaded_) {
	this.names = [];
	this.tutorialsFolder = tutorialsFolder_;
	this.tutorialsFile = tutorialsFile_;
	this.loaded = loaded_;
	this.txtLoader = new nanjizal_loaders_TxtLoader([this.tutorialsFolder + "tutorials.json"],$bind(this,this.jsonsLoaded));
};
nanjizal_tutorial_TutorialLoader.__name__ = true;
nanjizal_tutorial_TutorialLoader.prototype = {
	itemNames: function() {
	...
	,loadItemByName: function(name) {
		...
		var itemContent = this.items[index].content;
		var currentItem = itemContent.markdown;
		var f = $bind(this,this.renderMarkDown);
		var a1 = currentItem;
		var a2 = name;
		var a3 = itemContent.links;
		var tmp = function() {
			f(a1,a2,a3);
		};
		this.txtLoader.reload([this.tutorialsFolder + currentItem],tmp);
	}
	,jsonsLoaded: function() {
		var _this = this.txtLoader.contents;
		this.items = JSON.parse(__map_reserved["tutorials.json"] != null ? _this.getReserved("tutorials.json") : _this.h["tutorials.json"]).tutorials;
		this.loaded();
	}
	...
	,__class__: nanjizal_tutorial_TutorialLoader
};
